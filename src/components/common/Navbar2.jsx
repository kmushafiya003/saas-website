...
const Navbar2 = () => {
    ...
    return (
        <div className="container xl:px-4 px-1">
            <nav className="bg-white p-6 flex items-center justify-between flex-wrap">
                <div className="flex items-center justify-between w-full relative md:w-auto">
                    <img src={logo === 1 ? logo1 : logo2} alt="Logo" className="h-8 w-auto" />
                    <button
                ...
                <div className={`relative w-full md:flex md:items-center md:w-auto ${isMobileMenuOpen ? 'block' : 'hidden'} ml-8`}>
                    ...
                    <div className="flex flex-col md:flex-row md:space-x-6 mt-4 md:mt-0 items-center justify-center md:justify-start ml-8">
                        <Link to="/login" className={`px-8 py-4 hover:text-black mb-2 md:mb-0 ${logo === 1 ? "text-custom-orange cursor-pointer" : "text-secondary-light-turquoise"}`}>
                            Login
                        </Link>
                        <Link
                            to="/sign-up"
                            className={`text-white px-8 py-4 rounded-full hover:text-white focus:outline-none focus:ring-1 focus:ring-selective-yellow ${logo === 1 ? "bg-custom-orange hover:bg-orange-600" : "bg-secondary-light-turquoise"}`}
                        >
                            Sign Up
                        </Link>
                    </div>
